...
use crate::structure::{DiskMapping, TranslatorBehaviour};
use serde_json::json;
...
impl<'a> TranslatorBehaviour for JsonTranslator<'a> {
    ...
    fn from_disk(&self) -> serde_json::Value {
        let file = File::open(&self.disk_mapping.path).unwrap();
    ...
    fn to_disk(&self, database: &Vec<Table>) {
        println!("writing json to {:?}", &self.disk_mapping.path);
...
impl<'a> JsonTranslator<'a> {
    ...
    fn dump_json(&self, the_json: &serde_json::Value) -> Result<(), std::io::Error> {
        let path = self.disk_mapping.path.as_ref();
